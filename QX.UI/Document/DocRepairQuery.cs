...
{
    public partial class DocRepairQuery : Bse_Form
    {
        //public DocRepairQuery()
        //{
        //    InitializeComponent();
        //}
        ...
        void top_tool_bar_LookClicked(object sender, EventArgs e)
        {
            ...
            if (row != null)
            {
                string code = row.Cells["PRM_Code"].Value.ToString();
                var model = compInstance.GetPRMModel(code);
                ProdRepairOp frmOp = new ProdRepairOp(model, OperationTypeEnum.Look);
                frmOp.ShowDialog();
            }
        }
        ...
        void top_tool_bar_SearchClicked(object sender, EventArgs e)
        {
            ...
            dt = new BLL.Bll_Comm().ListViewData(string.Format("select * from VRpt_RepairQuery where 1=1 {0}", where));
            ...
            //    {
            //        case "SearchComp":
            //            where = string.Format(" AND PRDC_CompCode like '%{0}%'", val3);
            //            break;
            //        case "SearchPComp":
            //            where = string.Format(" AND PRDC_CompNo like '%{0}%'", val3);
            //            break;
            //        case "SearchProdCode":
            //            where = string.Format(" AND PRDC_ProdCode like '%{0}%' ", val3);
            //            break;
            //        default:
            //            where = string.Format(" AND ( PRDC_ProdCode like '%{0}%' OR PRDC_CompNo like '%{0}%' OR PRDC_CompCode like '%{0}%') ", val3);
            //            break;
            //    }
        ...
        public void InitControl()
        {
            ...
            comGrid = gen.GenerateGrid("CList_QueryRepair", this.pnlDoc, new Point(0, 0));
            comGrid.DoubleClickRow += new DoubleClickRowEventHandler(comGrid_DoubleClickRow);
            DataTable dt = new BLL.Bll_Comm().ListViewData(string.Format("select * from VRpt_RepairQuery where 1>2 "));
            comGrid.DataSource = dt;
        }
