...
namespace QX.GenFramework.Helper
{
    ...
    public class FTP
    {
        ...
        public bool UploadLocalFileWithOverWrite(string localPath)
        {
            try
            {
                string filename=Path.GetFileName(localPath);
                Response = Open(new Uri(this.Uri.ToString() + filename), WebRequestMethods.Ftp.UploadFile);
        ...
        public bool RenameDirectory(string FolderName, string newFolderName)
        {
            try
            {
                if (!IsValidFileChars(newFolderName) || !IsValidFileChars(newFolderName))
                {
                    throw new Exception("文件名非法！");
                }
                ...
                {
                    Request = OpenRequest(new Uri(this.Uri.ToString() + FolderName), WebRequestMethods.Ftp.Rename);
                    Request.RenameTo = newFolderName;
                    Response = (FtpWebResponse)Request.GetResponse();
                }
                else
                {
                    throw new Exception("文件夹在服务器上不存在！");
                }

                return true;
                //FtpWebRequest ftpReq = (FtpWebRequest)WebRequest.Create("ftp://" + _Host + filename);
                //ftpReq.Method = WebRequestMethods.Ftp.Rename;
                //ftpReq.Credentials = new NetworkCredential(_UserName, _Password);
                //ftpReq.RenameTo = newFolderName;
        ...
        public bool CopyFileToAnotherDirectoryOverWrite(string RemoteFile, string DirectoryName)
        {
            string CurrentWorkDir = this.DirectoryPath;
            try
            {
                byte[] bt = DownloadFile(RemoteFile);
                GotoDirectory(DirectoryName);
                bool Success = UploadFile(bt, RemoteFile, true);
        ...
        public bool CopyFileToAnotherDirectoryOverWriteEx(string RemoteFile, string DirectoryName)
        {
            string CurrentWorkDir = this.DirectoryPath;
            try
            {
        ...
        /// </summary>
        /// <param name="DirectoryName">子目录名称</param>
        public bool EnterOneSubDirectory(string DirectoryName)
        {
            try
            {
